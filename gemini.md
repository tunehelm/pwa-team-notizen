...
- `src/lib/localCache.ts` — IndexedDB-Cache via `idb`; `cacheFolders/cacheNotes/clearFoldersAndNotesCache`
...
### Seiten
- `DashboardPage` — Pinned + Root-Ordner, Avatar via `ownerProps()` + `profileMap` (aus `profiles`-Tabelle)
- `FolderPage` — Unterordner + Notizen eines Ordners
- `NotePage` — Editor (contenteditable), Smart Blocks, Bild-Upload, Toolbar
- `TeamHubPage` — Team-Chat + Mitgliederliste
- `TrashPage` — Papierkorb mit Restore/Delete
